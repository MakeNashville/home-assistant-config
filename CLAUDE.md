...
| `write_entity_list.sh` | Writes `entity_list.txt` and snapshots new entities into `entity_snapshots.json` |
| `entity_list.txt` | Auto-generated entity ID reference. **Do not edit by hand.** |
| `entity_snapshots.json` | Auto-generated mock entity states for local preview. **Do not edit by hand.** |
| `snapshot_blocklist.json` | Blocklist patterns for sanitizing sensitive entity data in snapshots |
| `validate.sh` | Local config validation and preview — run `./validate.sh --help` for usage |
...
Contributors can check their changes locally without needing the real integrations:

```bash
./validate.sh              # Config check only (same as CI) — requires Docker
./validate.sh --preview    # Boot a local HA instance with mock entity data
./validate.sh --preview --port 9123  # Custom port
```

**Config check** runs `check_config` via the official HA Docker image, using dummy secrets. Same validation as CI.

**Preview mode** boots a full HA instance with mock entity states injected from `entity_snapshots.json`. Dashboards render with realistic data, template sensors compute, and you can browse the UI at `http://localhost:8123` (login: `preview` / `preview`). Press Ctrl+C to stop.

- `entity_snapshots.json` is auto-generated by `write_entity_list.sh` on the HA instance and committed via the nightly backup PR
- `snapshot_blocklist.json` controls which entity values are sanitized (print filenames, task names, IP addresses, etc.)
- New entities are snapshotted only once (when first added to `entity_list`); existing entries are never overwritten
- Do not edit `entity_snapshots.json` by hand — it will be enriched by the HA instance
...
- Do not edit `entity_list.txt` or `entity_snapshots.json` by hand
